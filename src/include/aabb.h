...
class aabb {
	public:
		...
		bool intersect(const ray& r, interval ray_t) const {
			const point3d& ray_orig = r.origin();
			const vec3d& ray_dir = r.direction();
...
aabb operator+(const aabb& bbox, const vec3d& offset) {
	return aabb(bbox.x + offset.x(), bbox.y + offset.y(), bbox.z + offset.z());
}

aabb operator+(const vec3d& offset, const aabb& bbox) {
	return bbox + offset;
}
